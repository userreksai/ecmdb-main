...
import (
	...
	"github.com/userreksai/ecmdb-main/api/proto/gen/etask/executor/v1"
	"github.com/userreksai/ecmdb-main/api/proto/gen/etask/task/v1"
...
func InitModule(q mq.MQ, db *mongox.Mongo, orderModule *order.Module, workflowModule *workflow.Module, engineModule *engine.Module, codebookModule *codebook.Module, workerModule *worker.Module, runnerModule *runner.Module, userModule *user.Module, discoveryModule *discovery.Module, lark2 *lark.Client, crypto *cryptox.CryptoRegistry, sender2 sender.NotificationSender, taskClient taskv1.TaskServiceClient, executorClient executorv1.TaskExecutionServiceClient) (*Module, error) {
	taskDAO := InitTaskDAO(db)
	taskRepository := repository.NewTaskRepository(taskDAO)
	v := orderModule.Svc
	v2 := workflowModule.Svc
	v3 := codebookModule.Svc
	v4 := runnerModule.Svc
	v5 := engineModule.Svc
	v6 := userModule.Svc
	v7 := workerModule.Svc
	cryptoxCrypto := InitCrypto(crypto)
	taskDispatcher := dispatch.NewTaskDispatcher(v7, taskClient, taskRepository, cryptoxCrypto)
	v8 := discoveryModule.Svc
	schedulerScheduler := scheduler.NewScheduler()
	v9 := service.NewService(taskRepository, v, v2, v3, v4, v5, v6, taskDispatcher, v8, schedulerScheduler, sender2)
	handler := web.NewHandler(v9, executorClient)
	executeResultConsumer := initConsumer(v9, q)
	v10 := initStartTaskJob(v9)
	v11 := initPassProcessTaskJob(v9, v5)
	v12 := initTaskRecoveryJob(v9)
	v13 := initTaskExecutionSyncJob(v9, v5, executorClient)
	module := &Module{
		Svc:                  v9,
		Hdl:                  handler,
		c:                    executeResultConsumer,
		StartTaskJob:         v10,
		PassProcessTaskJob:   v11,
		TaskRecoveryJob:      v12,
		TaskExecutionSyncJob: v13,
	}
	return module, nil
}
