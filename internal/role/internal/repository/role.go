...
func (repo *roleRepository) CreateOrUpdateRolePermissions(ctx context.Context, code string, menuIds []int64, allowedModelUIDs []string) (int64, error) {
	return repo.dao.CreateOrUpdateRolePermissions(ctx, code, menuIds, allowedModelUIDs)
}
...
func (repo *roleRepository) toEntity(req domain.Role) dao.Role {
	return dao.Role{
		...
		Desc:             req.Desc,
		Status:           req.Status,
		AllowedModelUIDs: req.AllowedModelUIDs,
	}
}
